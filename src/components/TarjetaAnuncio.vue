      ...
      <v-row align="center" justify="center" class="fill-height" no-gutters>
        <v-col cols="12" lg="5">
          <v-carousel :height="tarjetaWH['carrusel']">
            <v-carousel-item v-for="(imagen,i) in anuncioView.Sec_Imagen" :key="i" :src="imagen.url" max-height="100%">
            </v-carousel-item>
          </v-carousel>
        </v-col>
        <!--Carrusel-->
        <v-col cols="12" lg="7" class="d-flex flex-column justify-center align-center align-stretch">
          <v-sheet class="d-flex" color="grey lighten-3" :height="tarjetaWH['cuerpoAnuncio']">
            <v-tabs class="d-flex flex-column-reverse">
              ...
              <v-tab-item v-if="anuncioView.Sec_Tarifas">
                <v-container fluid style="height: 440px; overflow:hidden;" class="pa-4 pa-lg-8">
                  ...
                  <v-row align="center" justify="center">
                    <v-slide-group v-model="categoriaSeleccionada" mandatory show-arrows center-active>
                      <v-slide-item v-for="(tarifa, i) in anuncioView.Sec_Tarifas" :key="i" v-slot="{active, toggle}">
                        <v-card width="210" class="ma-4" :color="active ? 'primary' : 'gray lighten-1'" @click="toggle">
                          <v-card-text>
              ...
              <!--taridas-->
              <v-tab v-for="(anuncio, key, i) in anuncioView" :key="i"
                v-if="key == 'Sec_Descripcion' || key == 'Sec_Contacto' || key == 'Sec_Tarifas'" style="height: 52px;">
                {{
